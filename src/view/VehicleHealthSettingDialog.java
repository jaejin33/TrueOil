...
public class VehicleHealthSettingDialog extends JDialog {
	...
	public VehicleHealthSettingDialog(Frame parent) {
		...
		card.setBorder(
				new CompoundBorder(new LineBorder(new Color(209, 213, 219), 2), new EmptyBorder(16, 24, 24, 24)));
	...
	private void addCycleSection(JPanel p, String title, int cycleVal) {
		...
		sectionPanel.setBorder(
				new CompoundBorder(new LineBorder(new Color(245, 245, 245), 1), new EmptyBorder(15, 15, 15, 15)));
	...
	private void loadCurrentSettings(JPanel formWrapper) {
		...
		for (MaintenanceStatusDto dto : list) {
			...
			int displayCycle = (dto.getCustomCycleMileage() == -1) ? dto.getCycleMileage()
