...
public class VehicleHealthDetailDialog extends JDialog {
	...
	public VehicleHealthDetailDialog(Frame parent, int itemId, String itemName, int lastKm, int cycle) {
		...
		addInput(formWrapper, "교체 날짜 (예: 2024-01-01)", replaceDateField = new JTextField(LocalDate.now().toString()),
				labelFont, fieldSize);
		addInput(formWrapper, "교체 시점의 주행거리 (km)", lastReplaceField = new JTextField(String.valueOf(lastKm)), labelFont,
				fieldSize);
		...
		saveBtn.addActionListener(e -> {
			try {
				...
				boolean success = maintenanceController.handleMaintenanceReplacement(this, itemId, itemName, date,
						mileage, cost);
